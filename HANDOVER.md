# PDW v5.1 2026 Release handover
...
- Active branch: `pdw-v5.1-2026-release`
- Product/display name: **PDW v5.1 2026 Release**
- Executable: `PDW v5.1 2026 Release.exe`
- Product version: `5.1.0 2026 Release`
- File version: `5.1.0.0`
- Installer: `PDW-v5.1-2026-Release-Setup.exe`
- Portable packages: `PDW-v5.1-2026-Release-Win32` and
  `PDW-v5.1-2026-Release-x64`

The v5.1 release is one PDW product. The installer contains both application
...
- Win32 PE machine `0x014C`, file version `5.1.0.0`, product version
  `5.1.0 2026 Release`;
- x64 PE machine `0x8664`, file version `5.1.0.0`, product version
  `5.1.0 2026 Release`; and
- embedded manifest version `5.1.0.0` with Per-Monitor V2 DPI awareness.

Content-free native UI review passed for the predecessor-v5 application on both
architectures. A v5.1 native Light/Dark and compact-size UI smoke remains open.
The shared resources and binary metadata now carry **PDW v5.1 2026 Release**;
...
The current v5.1 isolated installer smoke passes for x64 and Win32. It verifies application
architecture and version, settings co-location, upgrade preservation of
modified settings and custom receivers, removal of the exact renamed v5
predecessor executable, clean uninstall, and
...
cmake -S . -B out\v5.1-build-win32 -A Win32
cmake --build out\v5.1-build-win32 --config Release --target clean
cmake --build out\v5.1-build-win32 --config Release --parallel
ctest --test-dir out\v5.1-build-win32 -C Release --output-on-failure

.\scripts\build-dependencies.ps1 -Architecture x64
cmake -S . -B out\v5.1-build-x64 -A x64 `
  -DPDW_DEPENDENCY_ROOT="$PWD\out\dependencies\x64"
cmake --build out\v5.1-build-x64 --config Release --target clean
cmake --build out\v5.1-build-x64 --config Release --parallel
ctest --test-dir out\v5.1-build-x64 -C Release --output-on-failure

.\scripts\stage-installer-input.ps1 -Architecture Win32 `
  -BuildDirectory out\v5.1-build-win32\Release `
  -Destination out\v5.1-installer-input\Win32
.\scripts\stage-installer-input.ps1 -Architecture x64 `
  -BuildDirectory out\v5.1-build-x64\Release `
  -Destination out\v5.1-installer-input\x64
.\scripts\build-installer.ps1 `
  -Win32ApplicationDirectory out\v5.1-installer-input\Win32 `
  -X64ApplicationDirectory out\v5.1-installer-input\x64 `
  -OutputDirectory out\v5.1-installer
.\scripts\audit-installer.ps1 `
  -Setup 'out\v5.1-installer\PDW-v5.1-2026-Release-Setup.exe'
.\tests\installer_smoke.ps1 `
  -Setup 'out\v5.1-installer\PDW-v5.1-2026-Release-Setup.exe' `
  -TestRoot out\v5.1-installer-smoke
...
- Inspect and commit the complete v5.1 release diff.
- Push `pdw-v5.1-2026-release` to the writable `fork` remote.
