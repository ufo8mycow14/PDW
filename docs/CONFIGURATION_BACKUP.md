...
- every Capcode Directory mapping and filter rule, including labels, text matching, reject/monitor
  behavior, command/separate-file settings, colours and hit counters;
...
the active configuration and Capcode Directory transactionally. If a write fails, PDW attempts to
roll back to the original settings, directory and credentials. Backups created before the directory
upgrade remain supported: their legacy filter data is converted during restore. After a successful
restore PDW closes; reopen it to load the restored configuration cleanly. Credential records are
retargeted to the current PDW folder so a backup can be restored after PDW is moved or installed elsewhere.
